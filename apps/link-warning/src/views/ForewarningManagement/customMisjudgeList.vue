  ...
  setup() {
    ...
    const init = async () => {
      ...
      if (Object.keys(query).length > 0) {
        // 处理预警等级
        if (query.alarmLevel) {
          defaultForm.alarmLevel = query.alarmLevel
        }

        // 处理客户状态，如果有状态参数则覆盖默认值
        if (query.customerStatus) {
          defaultForm.customerStatus = [query.customerStatus]
        }

        // 处理时间类型
        if (query.timeType !== undefined) {
          const now = moment()
  ...
  <KyTreeTable ref="treeTable" :isShowLeft="false">
    ...
    <ECard slot="search" noneBottom type="search">
      <el-form :model="form" size="mini" inline>
        ...
        <el-form-item>
          <EButton type="primary" size="mini" @click="handleBatchProcess">
            {{ isBatch ? "取消批量处理" : "批量处理" }}
          </EButton>
        </el-form-item>
    ...
    <ECard slot="table" customStyle="box-sizing: border-box;overflow: hidden;">
      <div class="flex mx-2">
        <div class="mr-2 card-cell">
          <EButton type="primary" size="mini" @click="handleExport">
            Excel导出
          </EButton>
        </div>
        <BatchDeal
