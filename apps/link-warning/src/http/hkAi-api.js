...
export function getCameraListByHazard(companyId, hazardId) {
  const axios = createAxiosFromStore()
  return axiosPost(axios, `bind/getBindCameraInfo?companyId=${companyId}&hazardId=${hazardId}`)
}
// 摄像头点播
...
export function getAiHelper(params) {
  ...
  //   let param = ''
  //   if (params.cId) {
  //     param = `?cId=${params.cId}&q=${params.q}`
  //   } else {
  //     param = `?q=${params.q}`
  //   }

//   return axiosPost(axios, `agent/pushQWithCid${param}`, '', '', {
//     timeout: 300000,
//   })
}
