...
import GasNameDia from './components/gasNameDia'

export default {
  name: 'gasName',
  components: { GasNameDia },
  data() {
    return {
      isLoading: false,
      total: 0,
  ...
  methods: {
    ...
    changeFn(type, infoData) {
      ...
      switch (type) {
        case 'add':
          this.dialogTitle = '新增毒性气体名称校正系数'
          this.formData = {}
          break
        case 'edit':
          this.dialogTitle = '编辑毒性气体名称校正系数'
          this.formData = infoData
          break
        case 'view':
          this.dialogTitle = '查看毒性气体名称校正系数'
          this.formData = infoData
          break
      }
    },
    ...
    // 删除弹窗
    delFn(v) {
      this.$confirm('您确认要删除此毒性气体名称校正系数' + ' ?', '提示', {
    ...
    resetSearch() {
      ...
      this.searchData.businessCategory = 1
