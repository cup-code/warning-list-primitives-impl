...
import {
  deleteTianjianRecord,
  queryTianjianRecord,
} from "@/http/inspection/yj-inspection-api";
import TianjianInstallationRecordDialog from "./components/TianjianInstallationRecordDialog.vue";
import { TianjianInstallationRecordTableConfig } from "./config";

export default {
  name: "TianjianInstallationRecord",
  components: {
    TianjianInstallationRecordDialog,
  },
  setup() {
    ...
    // 表格数据
    const tableData = ref([]);
    const tableConfig = ref(TianjianInstallationRecordTableConfig);
    const total = ref(0);
    ...
    const { refetch, isFetching } = useQuery({
      queryKey: ["tianjianRecordList", searchForm],
      queryFn: () => queryTianjianRecord(buildQueryParams()),
      onSuccess: ({ data }) => {
        if (data?.success) {
          const { result } = data || {};
    ...
    const { mutate: deleteMutate } = useMutation({
      mutationFn: (id) => deleteTianjianRecord(id),
      onSuccess: (res) => {
        if (res.data?.success) {
...
<template>
  <div class="tianjian-installation-record-page">
