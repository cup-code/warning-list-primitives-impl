...
import { handleRuleCardByPage, handleRuleCardDel } from '@/http/defense/hubei/billCard-api'
...
import CardRuleInfo from './components/CardRuleInfo.vue'
...
export default {
  ...
  methods: {
    ...
    searchClick(isReqParams) {
      this.isLoading = true
      handleRuleCardByPage(this.searchData)
        .then((res) => {
          if (res.data.success) {
    ...
    editClick(info, editable) {
      if (editable) {
        this.dialogTitle = '编辑操作规程卡'
      }
      else {
        this.dialogTitle = '查看操作规程卡'
      }
      this.propData = {
        info,
        editable,
  ...
  <SearchTable v-loading="isLoading">
    ...
    >
      ...
      <el-form-item>
        ...
        <el-button
          v-if="hasBtnPermission('card_handle_rule_add')"
          type="primary"
          size="mini"
          icon="el-icon-plus"
          plain
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <el-button
            v-if="hasBtnPermission('card_handle_rule_view')"
            type="text"
          ...
          <el-button
            v-if="hasBtnPermission('card_handle_rule_modify')"
            type="text"
          ...
          <el-button
            v-if="hasBtnPermission('card_handle_rule_delete')"
            type="text"
