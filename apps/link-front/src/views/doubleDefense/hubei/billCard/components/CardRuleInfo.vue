...
import { getHandleRuleCardRichText, handleRuleCardSave } from '@/http/defense/hubei/billCard-api'
  ...
  methods: {
    ...
    getInfoData() {
      if (this.info.id) {
        ...
        // 获取富文本
        this.isLoading = true
        getHandleRuleCardRichText(this.info.id)
          .then((res) => {
            if (res.data.success) {
              this.changeData = {
    ...
    submitClick() {
      this.$refs.ruleForm.validate((valid) => {
        if (valid) {
          this.isLoading = true
          handleRuleCardSave(this.changeData)
            .then((res) => {
              if (res.data.success) {
