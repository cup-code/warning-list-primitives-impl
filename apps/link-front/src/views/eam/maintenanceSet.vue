...
export default {
  ...
  methods: {
      ...
      getMaintenancePlanList(this.sForm)
        .then((res) => {
          ...
          if (resD.success === true) {
            ...
            this.$message.error(msg || '查询保养失败')
          }
        })
        .catch((err) => {
          this.loading = false
          this.$message.error('查询保养失败')
        })
    },
    // 查询所有资产列表
    getAllAssetList() {
    ...
    getAllUsers() {
      ...
      getExecuteUsers('ROLE_MAINTENANCE').then((res) => {
        const resD = res.data
        if (resD.success === true) {
