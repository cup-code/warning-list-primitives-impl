    ...
    init(method, obj) {
      ...
      this.$nextTick(() => {
        ...
        if (method === 'edit' || method === 'view') {
          ...
          this.loading = true
          this.$http({
            url: `/test/treetable/testCarKind/queryById?id=${this.inputForm.id}`,
            method: 'get',
          }).then(({ data }) => {
            this.inputForm = this.recover(this.inputForm, data.testCarKind)
            this.loading = false
          })
        }
      })
          ...
          <el-col :span="12">
            ...
            >
              <SelectTree
                ...
                :url="`/test/treetable/testCarKind/treeData?extId=${inputForm.id}`"
