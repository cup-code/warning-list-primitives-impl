...
import {
  threeTimeRiskWasteProdByPage,
  threeTimeRiskWasteProdDel,
} from '@/http/pro-env/time3danger-api.js'
import DangerProdInfo from './components/DangerProdInfo.vue'
...
export default {
  ...
  data() {
    return {
      ...
      isLoading: false,
      total: 0, // 数据总数
      showProdDialog: false, // 是否打开新增/修改弹窗
  ...
  methods: {
    ...
    getDataList() {
      this.isLoading = true
      threeTimeRiskWasteProdByPage(this.searchData)
        .then((res) => {
          if (res.data.success) {
    ...
    delInfoClick(item) {
      this.$confirm(`您确定要删除第${item.$index + 1}条信息吗?`, '提示', {
        ...
        .then(() => {
          this.isLoading = true
          threeTimeRiskWasteProdDel(item.row.id)
            .then((res) => {
              if (res.data.success) {
...
<template>
  <SearchTable v-loading="isLoading">
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <span>{{ setDate(scope.row.produceTime) }}</span>
        </template>
      </el-table-column>
      <el-table-column
        label="危废分类"
    ...
    <div slot="dialog">
      ...
      <el-dialog
        class="normal-dialog"
        :visible.sync="showProdDialog"
        :close-on-click-modal="false"
        width="800px"
        :title="dialogTitlePro"
      >
        <DangerProdInfo
          v-if="showProdDialog"
          v-bind="proPropData"
