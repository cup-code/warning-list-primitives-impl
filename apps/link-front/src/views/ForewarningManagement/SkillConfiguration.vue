...
export default {
  ...
  setup() {
    ...
    const { refetch } = useQuery({
      queryKey: ["modelSkillList", "tenant", searchForm.value],
      queryFn: async () => await videoModelSkillTenantQuery(searchForm.value),
      onSuccess: (response) => {
        const { data } = response;
        if (data?.success) {
    ...
    const deleteSkill = useMutation({
      mutationFn: (id) => deleteVideoModelSkillTenant({ id }),
      onSuccess: (response) => {
        const { data } = response;
        if (data?.success) {
