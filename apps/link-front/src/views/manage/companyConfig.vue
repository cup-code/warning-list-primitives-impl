...
import YixunSetting from "./components/YixunSetting.vue";
    ...
    handleClick(event) {
      switch (Number.parseInt(event.index)) {
        ...
        case 1:
          ...
          break;
        case 2:
          this.itemCode = "YixunSetting"; // 两山易巡设置
          break;
        case 3:
...
<template>
  <TreeTable :isShowLeft="false">
    <ECard slot="table">
      <div class="text-lg font-medium">所属公司：{{ companyName }}</div>
      <div class="content">
        <el-tabs v-model="activeName" @tab-click="handleClick">
          ...
          <el-tab-pane label="两山易见" name="sixth">
            <VideoManagement
          ...
          <el-tab-pane label="两山易巡" name="seventh">
            <YixunSetting
              v-if="itemCode === 'YixunSetting'"
              :companyId="companyId"
              :itemCode="itemCode"
            />
          </el-tab-pane>
          <el-tab-pane label="特殊作业" name="second">
            <SpecialWork
          ...
          <el-tab-pane label="安环一张图" name="third">
            <OnePicture
          ...
          <el-tab-pane label="检验到期提醒设置" name="fourth">
            <RemainConfig v-if="itemCode === 'remainConfig'" :companyId="companyId" />
          </el-tab-pane>
          <el-tab-pane label="组织架构设置" name="fifth">
            <LeaderConfiguration
