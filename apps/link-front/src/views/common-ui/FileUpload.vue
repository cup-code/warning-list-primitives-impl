...
export default {
  props: {
    ...
    noticMsg: {
      type: String,
      default: "png/jpg/jpeg/word/excel/pdf/ppt/pptx",
    },
    // 是否允许上传所有文件类型（公共组件默认关闭，按页面显式开启）
    allowAllTypes: {
      type: Boolean,
      default: false,
    },
    // 是否使用自定义预览（由父组件通过 @preview 事件处理）
    useCustomPreview: {
      type: Boolean,
      default: false,
    },
  ...
  computed: {
    uploadAccept() {
      return this.allowAllTypes ? "*/*" : this.accept.toString();
    },
  },
  data() {
    return {
    ...
    uploadEvt(data) {
      return new Promise((resolve, reject) => {
        const allowAll =
          ...
          this.accept.includes("*");
        if (!allowAll && !this.accept.includes(data.file.type)) {
...
<template>
  <div v-loading="isLoading" class="file-box">
    <!-- 已上传文件列表 -->
    <div v-if="showFiles.length > 0" class="filelist-box">
      <div style="color: #67c23a">
        {{ uploadDes }}
      </div>

      <div v-for="(item, index) in showFiles" :key="item.id" class="filelist-item">
        ...
        <div class="filelist-item-right">
          <i
            ...
            @click="handlePreview(item.urlPath || item.filePath)"
          />
          <i
    ...
    >
      <el-button
        ...
        size="mini"
      >
        <i class="el-icon-upload" style="margin: 0 5px 0 0" />{{ btnDes }}
      </el-button>
    </el-upload>
  </div>
</template>
