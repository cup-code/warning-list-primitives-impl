<script>
import { emTeamGetByPage } from '@/http/emergency/emsource-api.js'
  ...
  methods: {
    ...
    pickClick(pickItem) {
      ...
      if (isExsit) {
        ...
        this.pickList.push({ id: pickItem.id, name: pickItem.name })
      }
    },
    /* 移除回调 */
    delTagClick(index) {
  ...
  >
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <span>{{
            $dictUtils.getDictLabelById('team_type', scope.row.teamType, '--', 'dictList')
          }}</span>
        </template>
      </el-table-column>
      <el-table-column
