...
import {
  ...
  reportList,
  reportUpdate,
} from '@/http/anji-report/reportmanage'
import { validateEngOrNum } from '@/utils/validate'
import MenuForm from '@/views/reportDesign/report/components/menuForm'
  ...
  methods: {
    ...
    preview(val) {
      ...
      if (val.reportType === 'report_excel') {
        routePath = '/excelreportViewer'
      }
      else {
        routePath = '/bigscreenViewer'
      }
    ...
    design(val) {
      let routePath = ''
      if (val.reportType === 'report_excel') {
        routePath = '/excelreportDesigner'
      }
      else {
        routePath = '/bigscreenDesigner'
      }
      const routeUrl = this.$router.resolve({
        path: routePath,
        query: {
...
<template>
  <anji-crud ref="listPage" :option="crudOption">
    <template slot="rowButton" slot-scope="props">
      <el-button type="text" @click="preview(props.msg)">
        预览
      </el-button>
      <el-button
        type="text"
