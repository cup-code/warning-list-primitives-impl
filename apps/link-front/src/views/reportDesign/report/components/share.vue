...
import { reportShareAdd } from '@/http/anji-report/reportShare'
...
export default {
  ...
  data() {
    return {
      ...
      shareLinkFlag1: true,
      rules: {
        shareValidType: [{ required: true, message: '有效期必选', trigger: 'change' }],
      },
    }
  },
  watch: {
  ...
  methods: {
    ...
    copyShare() {
      ...
      if (this.dialogForm.sharePassword == '') {
        content = `KY-Report分享链接：${this.reportShareUrl}`
      }
      else {
        content
          = `KY-Report分享链接：${this.reportShareUrl}  分享码：${this.dialogForm.sharePassword}`
      }
  ...
  >
    <div v-if="shareLinkFlag1">
      ...
      >
        ...
        <el-form-item
          label="分享码"
          prop="sharePasswordFlag"
        >
          <el-switch v-model="dialogForm.sharePasswordFlag" />
        </el-form-item>
      </el-form>
      <el-button
        type="primary"
    ...
    <div v-else>
      ...
      >
        ...
        >
          <el-input
            v-model="dialogForm.sharePassword"
            :disabled="true"
          />
        </el-form-item>
