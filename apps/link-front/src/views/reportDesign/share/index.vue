 ...
 * @Last Modified time: 2021-3-13 11:04:24
 ! -->
<script>
import { reportShareDetailByCode } from '@/http/anji-report/reportShare'
...
export default {
  ...
  methods: {
    async handleOpen() {
      ...
      const tenantCode = tempArr[tempArr.length - 2]

      const { code, data } = await reportShareDetailByCode(shareCode, tenantCode)
      if (code != '200')
        return
      console.log(data)
    ...
    checkPassword() {
      ...
      console.log(
        `${this.password}--------------${inputPassword}--------${this.sharePassword}`,
      )
