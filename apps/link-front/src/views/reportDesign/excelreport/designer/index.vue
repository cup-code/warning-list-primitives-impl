...
import {
  ...
  detailByReportCode,
  queryAllDataSet,
} from '@/http/anji-report/GaeaReport'
import { addReportExcel, editReportExcel } from '@/http/anji-report/report'
...
export default {
  ...
  async mounted() {
    ...
    try {
      ...
      console.log('✅ Luckysheet 加载完成，开始初始化报表设计器')
      this.loadDataSet()
      this.design()
    }
    catch (error) {
  ...
  methods: {
    ...
    onStart(setCode, evt) {
      ...
      const fieldLabel = evt.item.innerText // 列名称
