...
Vue.directive('dragx', (el, binding, vnode) => {
  ...
  const cfg = Object.assign({}, defaultOpts, binding.value)
  ...
  el.onmousedown = function (e) {
    ...
    function getStyleNumValue(key) {
      return Number.parseInt(style.getPropertyValue(key), 10)
    }
    const rect = el.getBoundingClientRect()
    const data = {
    ...
    document.onmousemove = function (edom) {
      ...
      if (isMove && cfg.canDrag) {
        ...
        let newLeft = Number.parseInt(getStyleNumValue('left') || '0', 10) + deltX
        let newTop = Number.parseInt(getStyleNumValue('top') || '0', 10) + deltY
        console.log(
          window.innerWidth + data.borderLeft + data.borderRight - data.width - cfg.resizeEdge,
        )
        if (newLeft <= 0) {
          newLeft = 0
        }
