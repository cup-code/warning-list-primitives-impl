<script>
import { ContractorDel, getContractorByPage } from '@/http/relevantContractor/contractor-api.js'
...
export default {
  ...
  methods: {
    ...
    searchClick() {
      this.isLoading = true
      getContractorByPage(this.searchData)
        .then((res) => {
          if (res.data.success) {
    ...
    editClick(infoId, editable) {
      const companyData = this.$refs.companyTree.getTreeData()
      if (editable) {
        this.dialogTitle = '修改承包商'
      }
      else {
        this.dialogTitle = '查看承包商'
      }
      this.propData = {
        infoId,
        companyData,
  ...
  <TreeTable v-loading="isLoading">
    ...
    >
      <el-form-item label="承包商名称">
        <el-input
          v-model="searchData.contractorName"
          placeholder="承包商名称"
        />
      </el-form-item>
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <span>{{ setDes(scope.row.contractorType, contractorTypeList) }}</span>
        </template>
      </el-table-column>
      <el-table-column
        label="承包商状态"
      ...
      >
        <template slot-scope="scope">
          <span>{{ setDes(scope.row.contractorStatus, contractorStatusList) }}</span>
        </template>
      </el-table-column>
      <el-table-column
        label="创建时间"
