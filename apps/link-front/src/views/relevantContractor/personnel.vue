<script>
import { getPersonnelByPage, personnerDel } from '@/http/relevantContractor/personner-api.js'
...
export default {
  ...
  methods: {
    ...
    searchClick() {
      this.isLoading = true
      getPersonnelByPage(this.searchData)
        .then((res) => {
          if (res.data.success) {
    ...
    editClick(infoId, editable) {
      const companyData = this.$refs.companyTree.getTreeData()
      if (this.editable) {
        this.dialogTitle = '修改承包商人员'
      }
      else {
        this.dialogTitle = '查看承包商人员'
      }
      this.propData = {
        infoId,
        companyData,
