/* * @Author: zhangbosen 首页代办 -- 演练计划审核 * @Date: 2023-11-27 */
<script>
...
export default {
  ...
  methods: {
    ...
    init(row, method) {
      ...
      this.getByIdDrillPlanFn(row.mainId)
    },
    getByIdDrillPlanFn(id) {
      getByIdDrillPlan(id).then((data) => {
        ...
        const docPath = data.result.docPath
        if (docPath) {
          this.fileProp.oldFileList = [
            {
              originalName: docPath,
              attachmentName: docPath,
              filePath: docPath,
            },
          ]
        }
