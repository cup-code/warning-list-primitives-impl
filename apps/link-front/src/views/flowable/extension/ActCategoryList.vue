        ...
        this.dataList = XEUtils.searchTree(
          ...
          (item) =>
            searchProps.some((key) =>
              XEUtils.toValueString(item[key]).includes(filterName)
            ),
          options
        );
        this.$nextTick(() => {
...
<template>
  <KyTreeTable ref="treeTable" :isShowLeft="false">
      ...
      >
        ...
        <vxe-table-column title="操作" width="300px" fixed="right" align="center">
          <template slot-scope="scope">
            ...
            <EButton type="text" icon="check" size="mini" @click="view(scope.row.id)">
              查看
            </EButton>
            <EButton type="text" icon="edit" size="mini" @click="edit(scope.row.id)">
              修改
            </EButton>
            <EButton type="text" size="mini" icon="delete" @click="del(scope.row.id)">
              删除
            </EButton>
            <EButton
              type="text"
    ...
    <!-- 弹窗, 新增 / 修改 -->
    <ActCategoryForm slot="dialog" ref="actCategoryForm" @refreshDataList="refreshList" />
  </KyTreeTable>
</template>
