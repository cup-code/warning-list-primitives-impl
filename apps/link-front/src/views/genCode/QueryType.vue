<script>
import { delQueryTypeFn, getQueryTypeListFn } from '@/http/safe-production/genCode/query-type-api'
import GenQueryTypeForm from './GenQueryTypeForm'
...
export default {
  ...
  methods: {
    ...
    refreshList() {
      ...
      getQueryTypeListFn(params).then(({ data }) => {
    ...
    // 新增
    add() {
      this.$refs.genQueryTypeForm.init('add', '')
    },
    // 修改
    edit(id) {
      id
      ...
      this.$refs.genQueryTypeForm.init('edit', id)
    },
    // 查看
    view(id) {
      this.$refs.genQueryTypeForm.init('view', id)
    },
    // 删除
    del(id) {
      const ids
