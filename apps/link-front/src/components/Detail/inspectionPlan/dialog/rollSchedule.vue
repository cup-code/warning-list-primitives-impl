/* * @Author: xiaorui 滚动排班的弹框 * @Date: 2022-07-15 11:37:23 * @Last Modified by: xiaorui *
@Last Modified time: 2023-02-16 10:09:39 */
<script>
...
export default {
  ...
  methods: {
    async init(planId, year, type) {
      ...
      getScheduleInfoByTypeFn(planId, year, type).then(({ data }) => {
        this.loading = false
        ...
        if (!data.result) {
          ...
          this.inputForm.duration = scheduleDetails[0].duration
          this.inputForm.expireHours = scheduleDetails[0].expireHours
    ...
    getExecuteStart(index) {
      const startHour
        = (Number(this.inputForm.startHour) + this.inputForm.duration * index) % 24 < 10
          ? `0${(Number(this.inputForm.startHour) + this.inputForm.duration * index) % 24}`
          : (Number(this.inputForm.startHour) + this.inputForm.duration * index) % 24
      return `${startHour}:${this.inputForm.startMinute}`
    },
    getExecuteEnd(index) {
      const startHour
        = (Number(this.inputForm.startHour) + this.inputForm.duration * index) % 24 < 10
          ...
          : (Number(this.inputForm.startHour) + this.inputForm.duration * index) % 24
      const endHour
        = (Number(startHour) + this.inputForm.duration) % 24 < 10
          ? `0${(Number(startHour) + this.inputForm.duration) % 24}`
          : (Number(startHour) + this.inputForm.duration) % 24
      return `${endHour}:${this.inputForm.startMinute}`
    },
    // 表单提交
    doSubmit() {
        ...
        if (valid) {
          ...
          const scheduleDetails = this.inputForm.scheduleDetails.map((item) => {
            return {
              ...
              executeEnd: `${this.inputForm.startHour}:${this.inputForm.startMinute}`, // 滚动排班同起始时间时间
