...
export default {
  ...
  computed: {
    // 主键的列名
    primaryKeyFieldName() {
      const primaryKey = this.option.columns.find(item => item.primaryKey == true)
      if (primaryKey != null) {
  ...
  methods: {
    ...
    fieldIsHide(editHide) {
      ...
      if (typeof editHide == 'string') {
        if (this.modelType == 'add') {
          return editHide.includes('hideOnAdd')
        }
        if (this.modelType == 'view') {
          return editHide.includes('hideOnView')
        }
        if (this.modelType == 'edit') {
          return editHide.includes('hideOnEdit')
        }
      }
      return false
    },
    ...
    fieldIsDisable(disable) {
      ...
      if (typeof disable == 'string') {
        if (this.modelType == 'add') {
          return disable.includes('disableOnAdd')
        }
        if (this.modelType == 'view') {
          return disable.includes('disableOnView')
        }
        if (this.modelType == 'edit') {
          return disable.includes('disableOnEdit')
        }
      }
      return false
    },
    ...
    async queryDetail() {
      const queryParams = this.relateData
  ...
  <div v-if="option.hide == null || option.hide == false" class="anji-card">
    <div class="card-head">
      ...
      <div class="main-card-header-button">
        ...
        <el-button type="text" @click="handleSetRowColNum(2)">
          ||
        </el-button>
      </div>
    </div>
          ...
          <el-row :key="group" class="form_table">
            <template v-for="item in groupFormFields[group]">
              <el-col
                ...
                :key="item.editField"
                :span="item.rowColSpan == null ? cardRowColSpan : item.rowColSpan"
              >
                <el-form-item
                  :label="item.label"
                  :rules="item.rules"
                ...
                >
                  ...
                  <anji-input
                    ...
                    :keep-point="item.anjiInput.keepPoint"
                    :rounding="item.anjiInput.rounding"
                    :placeholder="item.placeholder || '请输入'"
                    :clearable="item.clearable !== false"
                    :disabled="fieldIsDisable(item.disabled)"
                    @change="(value) => formChange(item.editField, value, null)"
                  ...
                  <anji-checkbox
                    ...
                    :disabled="fieldIsDisable(item.disabled)"
                    @change="
                      (value, options) => formChange(item.editField, value, options)
                    "
                  />
                  <!-- 城市三级联动 -->
                  <anji-cascader
