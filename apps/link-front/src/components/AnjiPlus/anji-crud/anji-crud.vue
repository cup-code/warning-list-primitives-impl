...
export default {
  ...
  computed: {
    ...
    // 表格中可展开的列
    tableExpandColumns() {
      const expandColumns = this.option.columns.filter(
  ...
  methods: {
    ...
    handleSortChange(column) {
      ...
      const sort = column.prop // 列表查询默认排序列
    ...
    handleQueryForm(from) {
      ...
      if (from == 'tree') {
        if (this.hasTreeFieldInQueryForm) {
          const treeVal = this.queryParams[this.queryFormTreeField.field]
          this.queryParams = {
            pageNumber: 1,
            pageSize: 10,
          }
...
<template>
  <div :class="[hasTreeFieldInQueryForm ? 'page-container' : 'app-container']">
    ...
    <div class="right-container">
      ...
      >
        <el-row>
          ...
          <el-col :span="6" style="text-align: center">
            ...
            <el-button type="danger" @click="handleResetForm()">
              重置
            </el-button>
            <a
      ...
      <el-button
        v-if="option.buttons.add.isShow == undefined ? true : option.buttons.add.isShow"
        type="primary"
        icon="el-icon-plus"
      ...
      <el-button
        v-if="
          option.buttons.delete.isShow == undefined ? true : option.buttons.delete.isShow
        "
      ...
      >
        ...
        >
          <template slot-scope="scope">
            ...
            <el-button
              v-if="
                hasRowCustomButton == false && option.buttons.delete.isShow == undefined
                  ? true
                  : option.buttons.edit.isShow
              "
              type="text"
