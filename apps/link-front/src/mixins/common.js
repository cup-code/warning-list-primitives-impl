...
export default {
  ...
  methods: {
    ...
    // 对象深拷贝
    deepClone(data) {
      const type = this.getObjectType(data)
      let obj
      if (type === 'array') {
        obj = []
      }
    ...
    objToOne(obj) {
      ...
      for (const index in obj) {
        if (typeof obj[index] == 'object') {
          const resObj = this.objToOne(obj[index])
    ...
    urlEncodeObject(obj, ingoreFields) {
      ...
      for (const key in obj) {
        ...
        if (ingoreFields != null && ingoreFields.includes(key)) {
          result[key] = obj[key]
        }
        else {
    ...
    getDictLabelByCode(dict, code) {
      const dictItem = this.getDictItemByCode(dict, code)
      if (dictItem != null) {
