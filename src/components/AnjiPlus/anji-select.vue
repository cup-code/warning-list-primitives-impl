...
export default {
  ...
  methods: {
    ...
    isDisabledOption(option) {
      ...
      const currentOptionVal = option[this.option]
      return this.disabledOptions.includes(currentOptionVal)
    },
    change(value) {
      if (value === '') {
        value = null
      ...
      // 根据当前值，找出对应的选项
      const optionItem = this.options.find(item => item[this.option] == value)
      this.$emit('change', value, optionItem)
    },
    // 根据用户配置的label，生成对应的标签
    getItemLabel(item, label) {
      if (!label.includes('${') && label.indexOf('}' < 0)) {
        return item[label]
      }
      const reg = /\$\{[a-z0-9]*\}/gi
      const list = label.match(reg)
      // ["${id}", "${text}"]
      let result = label
    ...
    getOptionsFromLocalStorage() {
      const dicts = getStorageItem('gaeaDict') || {}
      const options = []
      if (!dicts.hasOwnProperty(this.dictCode)) {
        return []
      }
      const dictItems = dicts[this.dictCode]
      for (let i = 0; i < dictItems.length; i++) {
        const dictItem = dictItems[i]
    ...
    queryData() {
      ...
      const options = this.getOptionsFromLocalStorage()
    ...
    queryDataByGet(keyword) {
      const param = this.deepClone(this.queryParam)
