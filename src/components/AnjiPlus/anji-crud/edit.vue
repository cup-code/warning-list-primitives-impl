...
export default {
  ...
  computed: {
    ...
    countJoinEntityOneToOne() {
      const entitys = this.joinEntitys.filter(item => item.joinType == 'OneToOne')
      if (entitys == null) {
        return 0
      }
      ...
      this.$refs.mainForm.validate((mainValid) => {
        ...
        for (var i = 0; i < this.joinEntitys.length; i++) {
          ...
          if (
            ...
            console.log(
              `已经校验的子表单：${this.countForValidJoinForm} 共：${this.joinEntitys.length}`,
            )
            // 所有关联表单校验通过
