<script>
import { chemistryOutDel, getChemistryOutByPage } from '@/http/major-hazard/ChemicalOutBook-api'
...
import outFrom from '@/views/majorHazard/components/ChemicalOutBookFrom'

export default {
  name: 'dangerChemicalOutBook',
  components: { CompanyTree, OutFrom: outFrom },
  data() {
    return {
      isLoading: false,
      total: 0,
  ...
  methods: {
    ...
    getTableData() {
      this.isLoading = true
      getChemistryOutByPage(this.searchData)
        .then(({ data }) => {
          if (data.success) {
    ...
    changeFn(type, infoData) {
      ...
      switch (type) {
        case 'add':
          this.dialogTitle = '新增危化品出库台账'
          this.fromData = {}
          break
        case 'edit':
          this.dialogTitle = '编辑危化品出库台账'
          this.fromData = infoData
          break
        case 'view':
          this.dialogTitle = '查看危化品出库台账'
          this.fromData = infoData
          break
      }
    },
