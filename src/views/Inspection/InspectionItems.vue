...
import {
  ...
  exportInspectionItemsList,
  getInspectionItemsList,
} from "@/http/inspection/inspection-items-api";
import InspectionItemsDialog from "./components/InspectionItemsDialog.vue";
import { InspectionItemsTableConfig } from "./config";
...
export default {
  ...
  setup() {
    ...
    const dialogType = ref("add"); // add-新增（绑定算法）, edit-编辑, view-查看
    const currentRow = ref({});

    // 表格配置
    const tableConfig = ref(InspectionItemsTableConfig);

    // 查询列表数据
    const { refetch, isPending } = useQuery({
      queryKey: ["inspectionItemsList", searchForm],
      queryFn: () => getInspectionItemsList(searchForm),
      onSuccess: ({ data }) => {
        const { result } = data || {};
        if (data?.success) {
          // 添加序号
    ...
    const { mutate: deleteMutate } = useMutation({
      mutationFn: (data) => deleteInspectionItems(data),
      onSuccess: (res) => {
        if (res.data.success) {
    ...
    const handleExport = () => {
      ...
      exportInspectionItemsList(searchForm)
        .then((res) => {
          ...
          link.href = url;
          link.download = `巡检项列表_${new Date().getTime()}.xlsx`;
          link.click();
  ...
  <KyTreeTable ref="treeTable" :isShowLeft="true">
    ...
    <ECard slot="table">
      ...
      <CTable :tableData="tableData" :loading="loading" :list="tableConfig" height="92%">
        <!-- 巡检算法列 - 展示省略效果 -->
        <template #algorithmNames="{ info }">
          <span :title="info.algorithmNames">{{ info.algorithmNames || "-" }}</span>
        </template>
