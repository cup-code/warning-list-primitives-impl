<script>
import {
  getSafeCheckTableTypeTree,
  safeCheckTableTypeDel,
  safeCheckTableTypeSave,
...
export default {
  ...
  data() {
    return {
      ...
      propData: {
        title: '检查表类型',
        getTreeFunc: getSafeCheckTableTypeTree,
        saveFunc: safeCheckTableTypeSave,
        delFunc: safeCheckTableTypeDel,
        isAdd: this.hasBtnPermission('safe_table_type_add'),
        isEdit: this.hasBtnPermission('safe_table_type_modify'),
        isDel: this.hasBtnPermission('safe_table_type_delete'),
      },
      allNodeList: [],
      editFormData: {},
  ...
  methods: {
    ...
    showDialogEvt(params) {
      if (params) {
        ...
        if (params.name) {
          this.dialogTitle = '检查表类型详情'
        }
        else {
          this.dialogTitle = '新增检查表类型'
        }
