...
export default {
  data: () => ({
    ...
    exForm: {}, // 后台导出 筛选表单
    hasCopyData: false, // 判断是否有复制的保养标准内容，点击复制按钮后为true；点击新增之后设置为false，避免重复复制
  }),
  watch: {
    typeList(v) {
  ...
  methods: {
      ...
      getMainItemList(this.sForm)
        .then((res) => {
          ...
          if (resD.success === true) {
            ...
            this.$message.error(msg || '查询保养标准失败')
          }
        })
        .catch((err) => {
          this.loading = false
          this.$message.error('查询保养标准失败')
        })
    },
    // 查询所有资产类型列表
    getAllTypeList() {
    ...
    // 新增 保养标准
    addFn() {
      // 新增保养项目时如果有复制的保养标准，则显示复制的标准；如果没有，则清空
      if (this.hasCopyData) {
    ...
    copyFn(v) {
      ...
      this.form = Object.assign(
        {},
        {
          ...
          itemStandard: v.itemStandard,
          sort: v.sort,
        },
      )
      this.$message.success('复制保养项成功！')
    },
    // 删除 保养标准
    delFn(v) {
    ...
    submitFn() {
      this.$refs.form.validate((valid) => {
        ...
        this.form.maintenanceType = 'MAINTENANCE' // 维保类型为 保养
    ...
    // 后台导出 确定
    backExportDoFn() {
      this.exForm.maintenanceType = 'MAINTENANCE' // 保养
    ...
    <el-drawer :visible.sync="drawer" :with-header="false">
      ...
      <div class="drawer-con">
        ...
        >
          ...
          <el-form-item label="保养内容" prop="itemContent">
          ...
          <!-- 操作类型为 提示时 显示 -->
          <el-form-item
            v-if="form.operationType === 'TIPS'"
            label="保养标准"
            prop="itemStandard"
          >
            <el-input v-model="form.itemStandard" />
          </el-form-item>
