/* * @Author: xiaorui 作废文档列表页 * @Date: 2023-01-13 11:54:07 * @Last Modified by: xiaorui *
@Last Modified time: 2023-05-24 14:21:53 */
<script>
import SelectTree from '@/components/treeSelect/treeSelect'
import { getInvalidListByPageFn } from '@/http/file-manager/document-api'
...
export default {
  ...
  methods: {
    ...
    getTableData() {
      this.isLoading = true
      getInvalidListByPageFn(this.searchData)
        .then(({ data }) => {
          if (data.success) {
  ...
  <TreeTable>
    ...
    >
      ...
      >
        <template slot-scope="scope">
          <el-button
            v-if="scope.row.voidCertificate"
            type="text"
            @click="showFileWindow(scope.row.voidCertificate)"
          >
            查看
          </el-button>
          <el-tag v-else>
