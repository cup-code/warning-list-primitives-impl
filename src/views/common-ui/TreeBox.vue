...
export default {
  ...
  methods: {
    ...
    getDataToTree(result) {
      const { id, pid, children } = this.defaultProps;
    ...
    >
      <div slot="header" class="space-between">
        ...
        <div>
          ...
          />
          <i class="tree-btn el-icon-refresh-right" @click="refreshClick" />
        </div>
      </div>
      <div style="height: 100%; width: 100%">
        ...
        >
          <span slot-scope="{ node, data }" class="space-between tree-item">
            <!-- <span :class="{ disabled: data.onlyTreeUse }">{{ data[defaultProps['label']] }}</span> -->
            <el-tooltip :content="data[defaultProps.label]" placement="top">
