  ...
  methods: {
    ...
    afterLogin(res) {
      ...
      if (resD.success) {
        ...
        localStorage.setItem('tk', res.headers.authorization)
        const saveSet = {}
        ...
        // 获取到用户菜单权限，转为tree类型
        console.log(result.permissions, 666)
        const permsTree = setPermissionTreeData(result.permissions)
        // .filter(
        //   item => item.menuName === '视频运营' || item.menuName === '运营管理',
        // )

        // permsTree.map((item) => {
        //   if (item.menuName === '运营管理') {
        ...
        //           ['企业菜单', '集团配置', '运营字典', '菜单管理'].includes(
        //             child.menuName,
        //           ),
        //         )
        //       }
        //       if (item.menuName === '系统管理' || item.menuName === '租户管理') {
        //         item.children = item.children.filter(child =>
        //           [
        //             '组织架构',
        //             '用户管理',
  ...
  <div>
    ...
    <div v-else class="loginOne-container">
      ...
      <div class="rounded-xl box-card">
        <div class="hasInfo">
          <div class="mb-3 font-sans text-4xl font-medium">
            登录
          </div>
        ...
        >
          <el-form-item prop="username">
            <div>
              <div class="mb-1 font-sans text-base font-normal text-black">
                账号
              </div>
              <el-input
