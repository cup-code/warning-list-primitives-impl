  ...
  methods: {
    init(method, id) {
      ...
      this.$nextTick(() => {
        ...
        if (method === 'edit' || method === 'view') {
          // 修改或者查看
          this.loading = true
          getValidateTypeDetailFn(this.inputForm.id).then(({ data }) => {
            this.inputForm = this.recover(this.inputForm, data.genValidateType)
            this.loading = false
          })
        }
      })
    ...
    doSubmit() {
      this.$refs.inputForm.validate((valid) => {
        if (valid) {
          this.loading = true
          saveValidateTypeFn(this.inputForm).then(({ data }) => {
