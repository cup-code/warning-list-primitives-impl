<script>
import {
  getQueryTypeDetailFn,
  saveQueryTypeFn,
} from '@/http/safe-production/genCode/query-type-api'
    ...
    init(method, id) {
      ...
      this.$nextTick(() => {
        ...
        if (method === 'edit' || method === 'view') {
          // 修改或者查看
          this.loading = true
          getQueryTypeDetailFn(this.inputForm.id).then(({ data }) => {
            this.inputForm = this.recover(this.inputForm, data.genQueryType)
            this.loading = false
          })
        }
      })
