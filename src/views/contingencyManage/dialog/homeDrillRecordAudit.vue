/* * @Author: zhangbosen 首页代办 -- 演练记录整改 * @Date: 2023-11-28 */
<script>
...
export default {
  ...
  data() {
    return {
      ...
      userData: {},
      staList: [
        { id: 1, dictCode: true, dictName: '已完成' },
        { id: 2, dictCode: false, dictName: '未完成' },
      ],
    }
  },
  created() {
  ...
  methods: {
    ...
    init(row, method) {
      ...
      this.getByIdDrillRecordFn(row.mainId)
          ...
          if (this.method === 'check') {
            this.$refs.inputFormCheck.validate((valid) => {
              if (valid) {
                ...
                params.problemId = this.businessId
                const {
                  id,
                  evidencePath,
                  problemId,
    ...
    >
      ...
      <el-form-item label="上传证据">
        <FileUpload
          v-if="visible"
          v-bind="accessory"
          @upload="uploadEvt($event, 'evidencePath')"
          @delSucc="delDocPath1"
        />
      </el-form-item>
    </el-form>
