...
export default {
  ...
  data() {
    return {
      ...
      grade: false,
      layerWidget: [],
      widgetTools, // 左侧工具栏的组件图标，将js变量加入到当前作用域
  ...
  computed: {
    ...
    middleWidth() {
      ...
      const middleWidth = this.bodyWidth - widthLeftAndRight
      return middleWidth
    },
    middleHeight() {
    ...
    bigscreenScaleInWorkbench() {
      const widthScale = (this.middleWidth - this.widthPaddingTools) / this.bigscreenWidth
      const heightScale = (this.middleHeight - this.widthPaddingTools) / this.bigscreenHeight
  ...
  methods: {
    ...
    widgetOnDragged(evt, widgetCode) {
      const widgetType = widgetCode

      // 获取结束坐标和列名
      const eventX = evt.originalEvent.clientX // 结束在屏幕的x坐标
      const eventY = evt.originalEvent.clientY // 结束在屏幕的y坐标
      ...
      // 复制一个组件
      const tool = getToolByCode(widgetType)
      const widgetJson = {
        type: widgetType,
        value: {
          setup: {},
...
<template>
  <div class="layout">
    ...
    >
      ...
      >
        <el-tab-pane
          v-if="isNotNull(widgetOptions.setup) || isNotNull(widgetOptions.collapse)"
          name="first"
          label="配置"
        >
          <dynamicForm
