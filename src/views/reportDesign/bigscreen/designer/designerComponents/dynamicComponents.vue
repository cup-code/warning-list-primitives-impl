...
import { detailBysetId, queryAllDataSet } from '@/http/anji-report/bigscreen'
...
export default {
  ...
  methods: {
    ...
    async echoDataSet(val) {
      ...
      this.dataSetValue = this.dataSet.filter(el => setCode == el.setCode)[0].id
...
<template>
  <div>
    ...
    >
      ...
      >
        <el-input
          v-model.trim="item.sampleItem"
          size="mini"
        />
      </el-form-item>
      <el-form-item
        v-for="item in setParamList"
        :key="item"
        :label="item"
      >
        <Dictionary
