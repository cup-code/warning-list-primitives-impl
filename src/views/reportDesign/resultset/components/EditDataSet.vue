...
export default {
  name: 'Support',
  components: { Dictionary, Codemirror: codemirror, VueJsonEditor: vueJsonEditor, MonacoEditor },
  props: {
    visib: {
      required: true,
      type: Boolean,
  ...
  methods: {
    ...
    reduceFilter(item) {
      if (this.itemFilterList.length > 0) {
        const index = this.itemFilterList.indexOf(item)
        if (index > -1) {
    ...
    async testResultset() {
      ...
      const {
        ...
        message,
        data,
      } = await verificationSet(this.isRowData)
      if (code == '200') {
        if (data) {
  ...
  <div>
    ...
    >
      ...
      >
        ...
        <el-row v-if="setType == 'sql'" :gutter="10">
          <el-col
            :xs="24"
            :sm="20"
            :md="22"
                ...
                <el-tab-pane label="查询参数" name="first">
                  ...
                  <el-table :data="tableData" style="width: 100%">
                    ...
                    <el-table-column label="描述" align="center">
                      <template slot-scope="scope">
                        <el-input v-model.trim="tableData[scope.$index].paramDesc" />
                      </template>
                    </el-table-column>
                      ...
                      <div class="filterTable">
                        ...
                        >
                          <div class="box">
                            ...
                            <el-button
                              v-if="
                                item.transformType == 'js' || item.transformType == 'dict'
                              "
                              type="text"
                              class="editor"
                              icon="el-icon-edit"
                            ...
                            <div v-for="(item, index) in tableData2" :key="index">
                              ...
                              >
                                <el-table-column label="key" width="180">
                                  <template slot-scope="scope">
                                    <el-input
                                      v-model.trim="item.children[scope.$index].name"
                                    />
                                  </template>
                                </el-table-column>
                                <el-table-column label="值">
                                  <template slot-scope="scope">
                                    <el-input
                                      v-model.trim="item.children[scope.$index].value"
                                    />
                                  </template>
                                </el-table-column>
                                <el-table-column
