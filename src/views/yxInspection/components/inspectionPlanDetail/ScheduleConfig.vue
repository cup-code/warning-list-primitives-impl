 ...
 * @Author: xiaorui
 * 巡检计划详情页的计划排班
 * @Date: 2022-05-07 17:27:55
 */
<script>
import {
  getInspectionPlanById,
  getScheduleByPlanIdAndYear,
  generateTask,
} from "@/http/inspection/yx-inspection-api";
import PersonScheduleDialog from "./PersonScheduleDialog.vue";
import PostScheduleDialog from "./PostScheduleDialog.vue";
import RollScheduleDialog from "./RollScheduleDialog.vue";

export default {
  name: "YxInspectionPlanScheduleConfig",
  components: {
        ...
        .then(({ data }) => {
          ...
          if (data.result && data.result.length) {
            data.result.forEach((item, index) => {
              if (item.scheduleType === "ROLL" || item.scheduleType === "滚动排班") {
                item.executeStart = this.getExecuteStart(
                  item.executeStart,
                  item.duration,
                  index
                );
                item.executeEnd = this.getExecuteEnd(
                  item.executeEnd,
                  item.duration,
                  index
                );
  ...
  <div class="schedule-config">
    <el-row>
      <el-col :span="14">
        ...
        <el-button type="success" :disabled="method === 'view'" @click="generateSchedule">
          生成任务班次
        </el-button>
      </el-col>
      <el-col :span="10" class="btnArea">
        ...
        <el-button type="primary" style="margin-left: 10px" @click="searchFn">
          查询
        </el-button>
    ...
    >
      ...
      <el-table-column prop="scheduleType" align="center" label="类型">
      ...
      <el-table-column prop="postName" align="center" label="岗位角色" />
      <el-table-column prop="executeUserNameList" align="center" label="巡检人">
        <template slot-scope="scope">
