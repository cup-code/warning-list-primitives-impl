  ...
  methods: {
    async init(planId, year, type) {
      ...
      if (data.result) {
        ...
        // 获取每行岗位对应的用户
        const userPromises = scheduleDetails.map((item) => getUsersByPostFn(item.postId));
  ...
  >
    ...
    >
      <el-form-item label="排班形式">
        <el-input v-model="inspectionCycle" disabled style="width: 200px" />
      </el-form-item>
      ...
      <el-table v-loading="loading" :data="inputForm.scheduleDetails">
        <el-table-column type="index" label="序号" width="50" column-key="index" />
        <el-table-column
          prop="scheduleSort"
          align="center"
          label="班次"
        ...
        <el-table-column align="center" label="岗位角色/巡检人">
          <template slot-scope="scope">
            <el-select
