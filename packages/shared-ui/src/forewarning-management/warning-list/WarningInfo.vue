<script>
import { validateWarningListHost } from './warningListHostCapabilities.js'
...
export default {
  ...
  computed: {
    ...
    getStatus() {
      return (status) => {
        const statusList = this.host.getDictList(this.type)
        let text = ''
  ...
  created() {
    validateWarningListHost(this.host, 'WarningInfo')
