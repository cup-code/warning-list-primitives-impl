...
export default {
  ...
  methods: {
    // 加载表单缓存
    loadFormCache() {
      const cachedData = this.host.getStorage(CACHE_KEY);
      if (cachedData) {
        // 恢复表单数据
        ...
        this.$nextTick(() => {
          if (
            ...
            const searchForm = this.host.reportDate.getWeekRange(this.timeType, this.dateRange);
            this.$emit("search", searchForm);
          } else if (this.timeType !== "custom" && this.timeType !== undefined) {
            const searchForm = this.host.reportDate.getWeekRange(this.timeType, "");
            this.$emit("search", searchForm);
          }
        });
      }
    ...
    saveFormCache() {
      ...
      this.host.setStorage(CACHE_KEY, cacheData, CACHE_EXPIRE_TIME);
    },
    // 清除表单缓存
    clearFormCache() {
    ...
    fileChangeEvt(file) {
      if (file) {
        this.host.uploadImage(file, "USER_ICON_PATH", false).then(({ data }) => {
          if (data.success) {
    ...
    handleDateTypeChange(value) {
      ...
      this.timeType = value;
      this.dateRange = [];
      const searchForm = this.host.reportDate.getWeekRange(value, "");
      this.$emit("search", searchForm);
      this.$nextTick(() => {
        this.saveFormCache();
      });
    ...
    handleDateChange(value) {
      ...
      const searchForm = this.host.reportDate.getWeekRange(this.timeType, value);
      this.$emit("search", searchForm);
      this.$nextTick(() => {
        this.saveFormCache();
      });
