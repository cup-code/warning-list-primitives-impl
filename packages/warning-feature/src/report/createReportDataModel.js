...
export function createReportDataModel({ dateLib, getWeekday } = {}) {
   ...
   */
  function generateActualList(screenData, machineNumber) {
   ...
   */
  function generateStatsList(machineNumber, cameraNumber, cameraSkillNumber) {
    return [
   ...
   */
  function processTrendTableData(search, alarmTrend) {
    if (search.timeType == 0) {
      const startDate = createDate(search.alarmDateStart)
      return [{
        date: startDate.format('MM-DD'),
        week: resolveWeekday(startDate.format('MM-DD')),
   ...
   */
  function processAlarmTypeRank(alarmTypeRank) {
   ...
   * @returns {Array} 处理后的排名数据
   */
  function processCameraAlarmRank(cameraAlarmRank) {
    if (!cameraAlarmRank)
      return []
   ...
   */
  function processAlarmLevelRank(alarmLevelLive, total) {
    const level = [
      { name: '一级预警', value: 1 },
      { name: '二级预警', value: 2 },
      { name: '三级预警', value: 3 },
   ...
   * @returns {string} 报告文本
   */
  function generateReportText(cameraNumber, cameraSkillNumber) {
