...
3. 在临时克隆中改写两份历史，删除所有提交里的 `.npmrc`、`node_modules`、
   `dist`、构建缓存和 `.DS_Store`，同时保留作者、时间、提交说明和提交拓扑。
4. 记录原提交 SHA 与净化后 SHA 的映射。历史改写会改变提交 SHA，这是本方案
   接受的结果。
5. 把净化后的历史分别导入 `apps/link-front` 和 `apps/link-warning`。
6. 将两个来源项目当前工作目录的内容覆盖到对应应用目录，复制时使用同样的排除项。
7. 在新仓库提交工作目录快照和 Monorepo 配置。

这样既能保留有效源码历史，也不会把历史中的认证信息和生成物带入新仓库，同时
还能包含尚未提交的最新代码。开始提取共享代码之前，需要对比来源目录和复制结果
的文件清单，确认没有遗漏或多复制文件。
...
- 新仓库能够访问两个来源项目净化后的源码历史，并保存完整 SHA 映射。
- 两个新应用包含来源工作目录中的当前内容。
- 迁移过程没有改变两个来源目录的文件内容。
